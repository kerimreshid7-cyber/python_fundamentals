...
# I think there is no need to explain(write here) every datail that what i have understood bc it takes time i have to focus on practice. as i am tecnology student(SWE) i understand the logic behind.
...
print("next activities\n\t1 using chat gpt make business logic challenges\n\t2 practice more BL with python\n\t3 be smart in learning merge every thing (python,sql,business logic) to become BI professional.")
#  there is another way and it's look like more professional and i am happy to learn by barra
print("""next activities
      \t1 using chat gpt make business logic challenges
      \t2 practice more BL with python
      \t3 be smart in learning merge every thing (python,sql,business logic) to become BI professional.""")
# now here we change the quots to 3 3 and remove (\n) bc of it gonna create extra line gaab bc we use new line to write so that is enough.
...
x=x+3
name="kerim"
print(name,"has started practicing python basics bc \"practice makes perfect\" Ronaldo said.")
print("my name is",name)
  # so here we can affect all prints with name by update only the assignment name="kerim" for ex. if we change to name="ebrahim" then every name in print will change.
  #  and if we have repited word also we can use variable and this is one of the most importance of variables in real projects. so update become easy with variables.
