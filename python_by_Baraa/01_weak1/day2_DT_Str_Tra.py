    ...
    #today I am gonna cover data type,string, and transformation
age=18

Name="kerim reshid" 
email='kerim15.gamil.com'
nicname="7"        #these three are string while having integers but if the numbers are inside quotes are considered as str.
id=1723                #where int stores this numbers? not in the air but python gonna store and connect as an object to class integer.
...
print("your age is: "+str(age)) # to print text and changing the integer to string then print.
...
name=""           # str - blank  It's complitly different from None bc python treate this as string,will create memory space and we call it blank(empty string)
...
"function": "do some thing on the data",
  "data":'is raw facts that we are gonna to analyse, change to info(organized data) or insight even wisdom to solve real life problems.'
}            
...
# the difference bn function and methode 
...
# method is a function that belongs to object inside a (class)
print("SOMETHING".lower())          #first value then function name with parentesis   this is syntax of method.
...
print(id.bit_length())    #so now we can see the length of the id in bit 
 ...
 #  data types  decide which opration or function is allowed to data values so when we use inappropriate function it return error with discription.
...
#    3 multiple (set,tuple,array,lists,arrays and dictionaries)
#  I will cover these all one by one in detail. the most is string so let me start with string 
 ...
 # so, we have to proccess analyse transform those datas. understanding string bhind the sign is 
...
# age=age-4         # so now in syntax it's correct but it's error in the out put bc we can't use opratotores for  str and int.
...
# This method will show us 2 on the out put why kerim is appeare 3 time in the text bc python is very case sensetive. the one Kerim is start by capital letter.
