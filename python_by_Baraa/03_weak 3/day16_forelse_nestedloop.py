...
# challenge 5 Scenario: Sales Performance by Region and Product
# You have sales data for multiple regions and multiple products. You want to find which products in which region have sales above a threshold.
# Sample data
sales_data = {
    "North": {"Laptop": 120, "Tablet": 90, "Phone": 150},
    "South": {"Laptop": 80, "Tablet": 60, "Phone": 200},
    "East": {"Laptop": 150, "Tablet": 100, "Phone": 180},
    "West": {"Laptop": 70, "Tablet": 50, "Phone": 130}
}

# Threshold
threshold = 100

# Using nested for loops to check sales by region and product
for region in sales_data:
    for product, sales in sales_data[region].items():
        if sales > threshold:
            print(f"{product} in {region} region has high sales: {sales}")
