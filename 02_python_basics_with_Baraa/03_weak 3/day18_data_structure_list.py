...
# list [store different data typs]                     empty list       [] and list()
# set{stores unique data value}                        empty set        set()
# dict{store datas with keys}                          empty dict       {} and dict()
...
# name,age,roll,country=employee                # and this is how we can unpack easily in pro way.
# print(f"name:{name} age:{age} years old")     # why do we need unpacking?
# # bc if we want to apply the functions like aggregation,transformation and so on we hve to assign each values to their data type.
...
# print(name,details)               # and this is how we can  pack in list.
